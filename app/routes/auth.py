...
def sign_up(): # 회원가입 처리 함수
    nickname = request.form.get('user_id') # 그 html 파일내에 폼 태그에서 가져오기
    password = request.form.get('user_pw') # 얘도 같은 방식
...
def login_check(): # 로그인 체크 함수
    user_id = request.form.get('user_id') # 회원가입때랑 같은 형식으로 form 태그에서 가져오기 
